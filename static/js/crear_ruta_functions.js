...
function renderizarListaDestinos(destinosFiltrados) {
    ...
    if (destinosFiltrados.length === 0) {
        lista.innerHTML = '<p class="text-sm text-gray-500 text-center py-4">No se encontraron destinos</p>';
        return;
    }
    ...
    destinosFiltrados.forEach(function(dest) {
        ...
        var div = document.createElement('div');
        div.className = `destino-item p-3 rounded-lg border ${isSelected ? 'selected border-sky-300' : 'border-gray-200 bg-white'}`;
        div.innerHTML = `
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <h4 class="text-sm font-semibold text-gray-900">${dest.nombre}</h4>
                    ...
                    ${dest.precio_promedio_minimo && dest.precio_promedio_maximo ? 
                      `<p class="text-xs text-green-600 mt-1"><i class="fas fa-dollar-sign mr-1"></i>${dest.precio_promedio_minimo.toFixed(1)} - ${dest.precio_promedio_maximo.toFixed(1)}</p>` : ''}
                </div>
                <div>
                    ${isSelected ? 
                        '<span class="inline-block bg-sky-100 text-sky-700 text-xs px-2 py-1 rounded-full font-semibold"><i class="fas fa-check"></i></span>' : 
                        ''
                    }
                </div>
            </div>
